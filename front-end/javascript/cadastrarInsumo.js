...
const formCadastro = document.getElementById('cadastrarInsumo');
...
async function carregarEstoque() {
    try {
        const response = await fetch('http://localhost:3000/saude/estoque');
        const result = await response.json();
...
formCadastro.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        const response = await fetch('http://localhost:3000/saude/cadastro', {
            method: 'POST',
            headers: {
...
async function carregarInsumos() {
    try {
        const response = await fetch('http://localhost:3000/saude/insumos');
        const result = await response.json();
...
formEntrada.addEventListener('submit', async (event) => {
    ...
    try {
        const response = await fetch('http://localhost:3000/saude/entrada', {
            method: 'POST',
...
document.querySelector('#form-saida').addEventListener('submit', async (event) => {
    ...
    try {
        const response = await fetch('http://localhost:3000/saude/saida', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ insumo, validade, quantidade, fazenda })
        });
